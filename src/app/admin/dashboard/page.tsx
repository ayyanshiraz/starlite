...
export default async function AdminDashboard() {
  ...
  return (
    <AdminGuard requiredPermission="dashboard">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {recentOrders.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-gray-400 italic">No orders found yet.</td>
                  </tr>
                ) : (
                  recentOrders.map((order) => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                        <td className="p-4 font-mono text-xs text-gray-500">#{order.id.slice(-8).toUpperCase()}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 font-bold text-gray-900">{formatMoney(order.amountTotal)}</td>
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${
                            ...
                            status === 'REFUNDED' ? 'bg-purple-100 text-purple-700 border-purple-200' :
